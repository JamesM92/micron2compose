...
- **Headings no longer render bold at a scaled-up font size** — that had no basis in real Micron rendering at all (a terminal UI has no font-size concept, and NomadNet's own heading styles are explicitly `bold: false`). Replaced with the actual upstream convention: a distinct foreground/background color band per level (1–3), ported directly from `MicronParser.py`'s `STYLES_DARK`, spanning the full row with the heading text indented inside it. Found via a real on-device screenshot from `nomadportal-android`.
